...
Install and manage Deathloop mods from one application. Supports the inspected Steam and Epic Windows 1.820.5.1 builds. Starts the selected executable directly and allows for store-client process handoff. Steam is not required for an Epic installation.
...
6. Stay at the main menu until the manager confirms that your mods are active.
...
Unchecking disables a mod but keeps it installed. Removing moves its files into the manager's `removed-mods` folder for recovery. You can permanently delete those archived files later. You do not need to change the game's DLLs to remove an individual mod. The list refreshes automatically after changes.
...
Use the checkboxes beside installed mods. Your choices are saved automatically. Installing a mod does not enable it.
...
A profile is simply a named selection of installed mods. Most people can keep using **Default**. Select a profile from the dropdown; its checkboxes are saved automatically.
...
Private mixed-mod/vanilla invitations are not supported by this release.

### Comparing setups when players cannot match

Each launch automatically writes a `launch-*.log` file containing the selected setup and launch result. Open **Tools → Logs** and compare the latest launch logs with the other player. Runtime logs record activation details. There is no separate sharing button.
...
Version 0.3 uses compatibility protocol `dlmods-v2`. Both players must use the new manager; older `dlmods-v1` launches use a different matchmaking pool. Existing mod ZIPs and profiles remain usable.

## Startup support

The manager installs its own startup support automatically. For modded launches, it disables the game's built-in protection before the game initializes and selects the modded matchmaking pool. No separate protection-disabling download is needed.

For vanilla launches, the manager removes its startup DLL from the active `dinput8.dll` filename, keeping a recoverable `dlmod-disabled-<hash>.dll` backup. Unknown DLLs are not changed. Older third-party startup DLL backups are not automatically restored. Modded launch requests are single-use and expire after five minutes.
...
Use manager 0.3.3 or later for Epic. This fixes an input-forwarding loop involving Epic's overlay, without disabling the overlay. The manager copies this computer's Windows input library to `dlmod-system-input.dll` beside the game. It is not downloaded or bundled in the release ZIP. Unknown files at that path are not overwritten. For All maps are invadable, install version 1.0.2 on both players' computers. The manager and mod are separate downloads. Local Epic startup and all 14 map changes have been verified; the players have also reported a successful Steam–Epic connection.

Manager 0.3.4 adds the Deathloop-inspired colors and application icon; its mod format and matchmaking protocol are unchanged.

Manager 0.3.5 uses the supplied orange emblem for its window and executable icon.

Manager 0.3.6 keeps its short-lived startup request available across the Steam launch handoff and records each startup process in the diagnostic log. The manager removes the request after runtime activation. This changes no mod package format or matchmaking pool identifier.

The manager does not edit your saves or replace `Deathloop.exe`. An existing recognized startup DLL is backed up before replacement. An unrecognized DLL is left untouched and reported as a conflict.
...
- `dlmod-system-input.dll`: a private-named copy of this computer's Windows input library, used to avoid double-hooking by overlays. It remains unused when the manager's active startup DLL is detached. `dlmod-system-input.sha256` records ownership so an updated Windows copy can safely replace it.
...
Install mods only from sources you trust. Mods can change game memory.

This release is a development preview. Keep backups before experimenting with new mods.
